...
class MnistDataSet(Dataset):
    def __init__(self):
        ...
        self.mnist_train_data = MNIST(root='data_temp', train = True, transform = transform, download = True)

    def __len__(self):
        return int(len(self.mnist_train_data)/4)
...
trainer_module = WassersteinGanGpModule(generator_ctor(), discriminator_ctor(), batch_size=BATCH_SIZE, metrics=metrics)
